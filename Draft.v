...
Proof.
  ...
  intro a.
  exact ((concat_p_pp (p:= (transport2 (λ _ : T f, P) (tp_1 a) (t' a))^)  (q:= transport_const (tp a a 1) (t' a)) (r:= tp' a a 1))                                                                                                 @ whiskerR (moveR_Vp _ _ _ (transport2_const (A:=T f) (B:= P) (tp_1 a) (t' a))) (tp' a a 1)                                                                                                         @ concat_1p _                                                                                     @ (tp_1' a)).
Defined.
...
Definition tp_bar `{ua: Univalence} {A B} (f: A -> B) (Mono_f : IsMono f) 
  : IsMono (t: A -> T f).
Proof.
  intros a b.
...
Section Foo.
  ...
  Proof.
    ...
    - revert e.
      ...
      apply (λ P Q f, ap10 (O_rec_retr n nj P Q f)).
      ...
      (* |[|- transport ?PP (?ff (i q)) ?gg = ?hh] => *)
       (* refine ((O_rec_dep _ (λ q, Build_subuniverse_Type n nj (BTT (transport PP (ff (i q)) gg = hh)) _)).1 q) *)
      (* end. *)
      apply admitt.
    - unfold Sect.
