...
# coq_makefile Utf8_core.v cech_nerve.v cloture_hpullback.v colimit.v epi_mono.v equivalence.v lemmas.v modalities.v nat_lemmas.v reflective_subuniverse.v reflective_subuniverse_O_different_level.v sheaf_base_case.v sheaf_def_and_thm.v sheaf_induction.v sub_object_classifier.v univalence.v -o Makefile COQBIN = /Users/tabareau/Softwares/HoTT/ 
#
...
COQBIN=/Users/tabareau/Softwares/HoTT/

OPT?=
COQDEP?="$(COQBIN)hoqdep" -c
COQFLAGS?=-q $(OPT) $(COQLIBS) $(OTHERFLAGS) $(COQ_XML)
COQCHKFLAGS?=-silent -o
COQDOCFLAGS?=-interpolate -utf8
COQC?=$(TIMER) "$(COQBIN)hoqc"
...
VFILES:=Utf8_core.v\
  ...
  nat_lemmas.v\
  colimit.v\
  sub_object_classifier.v\
  cech_nerve.v\
  reflective_subuniverse.v\
  modalities.v\
  sheaf_base_case.v\
  sheaf_def_and_thm.v\
  cloture_hpullback.v\
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Top && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Top" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
