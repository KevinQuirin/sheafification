...
Section hPullback.
  ...
  Lemma test_pullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) : hPullback m f 2 TrX TrY <~> pullback f f.
    ...
    - intros [b [c q]].
      exists (b,(c,tt)). 
      exact (q,tt).
    - intros [b [c q]]. reflexivity.
    - intros [[a [b tt]] [q r]]. destruct tt, r. reflexivity.
  Abort.
...
Section Old_pullback.
  ...
  Definition char_old_hPullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y)
  ...
  : (char_old_hPullback m f (S n) TrX TrY P).1 -> (char_old_hPullback m f n TrX TrY (forget_hProduct Y n P p)).1.
    induction n.
  ...
  Definition old_hPullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) : Type
    := {y : hProduct Y n & (char_old_hPullback m f n TrX TrY y).1}.
  
  Lemma trunc_old_hPullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y)
  : IsTrunc (trunc_S m) (old_hPullback m f n TrX TrY).
    induction n.
    - refine trunc_sigma. 
  ...
  Lemma test_pullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) : old_hPullback m f 2 TrX TrY <~> pullback f f.
  ...
  Definition proj_pullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) (P : old_hPullback m f n TrX TrY) : forall p:{p:nat & p < n}, Y.
    apply proj_hProduct. exact P.1.
  Defined.

  Definition forget_pullback (m:trunc_index) {X Y:Type} (f:Y -> X) (n:nat) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) (P : old_hPullback m f (S n) TrX TrY) : forall p:{p:nat & p <= n}, old_hPullback m f n TrX TrY.
    intros p.
  ...
  : old_hPullback m f (S 0) TrX TrY <~> Y.
    refine (equiv_adjointify _ _ _ _).
  ...
  : old_hPullback m f n TrX TrY -> hProduct Y n
    := pr1.
...
Section Cech_Nerve.
  ...
  Definition Cech_nerve_diagram (m:trunc_index) {X Y:Type} (f: Y -> X) (TrX : IsTrunc (trunc_S m) X) (TrY : IsTrunc (trunc_S m) Y) : diagram (Cech_nerve_graph f).
    ...
    intro n.
    exact (old_hPullback m f (S n) TrX TrY).
    intros i j; simpl in *.
