...
(* The universal property commute with η *)
  ...
  Lemma universality_unit_lemma (oA A B: Type) (η : A -> oA) (f g : oA -> B) (inv : (A -> B) -> oA -> B) (π : f o η = g o η) (eisretr : forall x:A -> B, (inv x) o η = x) (eissect : forall x : oA -> B, inv (x o η) = x) a
  : apD10 (ap inv π) (η a) = ((apD10 (eisretr (f o η)) a @ apD10 π a) @ (apD10 (eisretr (g o η)) a) ^)%path.
    destruct π. simpl. rewrite concat_p1. rewrite concat_pV. reflexivity.
  Defined.
...
(* Product *)
  ...
  Definition subuniverse_product' (A B : Trunk n) (TrP : IsTrunc n (A.1*B.1)) : (O subU (A.1*B.1 ; TrP)).1.1 = ((O subU A).1.1*(O subU B).1.1).
    ...
    - intros [oa ob]. simpl.
      ...
      unfold compose in rew; simpl in rew.
  ...
  Definition subuniverse_sigma :
    ...
    - intro H. intros A B g.
      ...
      assert ( (eq''' (η a)) =  (eqf a) ..1).
        ...
        rewrite universality_unit. unfold path_forall. rewrite eisretr. reflexivity.
  ...
  : (ap10
    ...
    pose (rew := @ap10_ap_precompose). unfold compose in rew. rewrite rew. reflexivity.
