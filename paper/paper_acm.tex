...
between the axiom of choice and the law of excluded middle in type theory\looseness=-1.
...
as Martin-Löf type theory, with univalence axiom (and thus functional
...
\begin{defi}
  For any type $A$ and $E$, $\iota : A \to E$ and $F:\Type_{n+1}$, we define
    $\Phi_E^\iota \defeq - \circ \iota : (E \to F) \to (A \to F) $
    as the map sending an  arrow $f:E\to F$ to its restriction $f \circ \iota$.
...
\begin{proof}
  ...
  where
  \begin{mymath}[0em]\phi : \left(\sum_{b:\left(\sum_{e:E} (\chi\, e)\right)} (f\,
