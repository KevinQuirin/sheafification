...
%
Finally, Section~\ref{sec:future-works} concludes on future works\looseness=-1.
...
universe-polymorphic way\looseness=-1.
...
\begin{prop}\label{prop:consistent}
  ...
  theory. In that case, the modality is said to be consistent\looseness=-1.
\end{prop}
\begin{proof}
...
sheaves that will be useful in the next section\looseness=-1. 
...
\begin{defi}
  For any type $E$, $\chi : E \to \Type$ and $F:\Type_{n+1}$, we define
    ...
    as the map sending an  arrow $f:E\to F$ to its restriction $f \circ \pi_1$\looseness=-1.
...
This proves the following proposition\looseness=-1
...
$\modal_n(x=y)$ is contractible.\looseness=-1
...
\TeX\xspace file contains approximatively 1800 lines\looseness=-1.
