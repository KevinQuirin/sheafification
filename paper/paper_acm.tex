...
\hyphenation{op-tical net-works semi-conduc-tor gro-then-dieck LT-sheafi-fi-ca-tion}
...
such that $\IsType n A$, either as a characteristic map $B\to \Type_n$.
% \begin{mymath}
% \xymatrix{
...
\begin{proof}
  ...
  \item For $n=-1$: Let $T:\HProp$. Then
    ...
    \begin{mymath}\phi t = (\lambda t',\, (t=\eta_T (t'))
      ;\kappa)\end{mymath}%
    where $\kappa$ is defined by $\modal_{-1}$-induction on $t$.
...
\begin{prop}\label{prop:univalence}
  ...
  $$A = B \to A\simeq B,$$
  then $\modal(\IsEquiv(\varphi))$ is inhabited.
\end{prop}
\begin{proof}
...
excluded middle is true, and which is univalent when restricted to
$n$-types (in the sense of proposition~\ref{prop:univalence}).
...
\|\cdot\|_n$ the constructed modality. We define the following
translation
\begin{itemize}
\item For $\Type$
\[
\begin{array}{lcl}
  \Lbrack \Type\Rbrack &\defeq& \Type_\modal
\end{array}
\]
...
As in~\cite{jaber2012extending}, the main issue is that $\Lbrack
U\{x/N\} \Rbrack$
and $\Lbrack U \Rbrack\{x/[N]\}$ are equal, but not convertible. We
therefore have to use $\mathrm{transport}$, the eliminator of the
equality, in the conversion rule.
...
\item It would be useful to give a syntactic translation of
  sheafification,
  as it has been done for forcing~\cite{jaber2012extending}, to be
