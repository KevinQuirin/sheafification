...
%
One use of sheafification is the construction a boolean
...
All those works---although they provide very nice and
sophisticated theoretical results---are not as general as the process
of sheafification in topos theory and are not amenable to a
...
as Martin-Löf type thepry, with univalence axiom (and thus functional
...
are part, or direct applications of the Coq library~\cite{hottlib}, while other definitions
...
\IsType n\
\fib{f}{b}\right) \\ &\qquad\xrightarrow{\sim} 
 (B \to \Type_n)
\end{align*}
%
...
\begin{mymath}t_f = \lambda a,~(\fib{f}{f(a)},(a,\mathrm{idpath})).\end{mymath}

One has a way, from any type $T$, to construct a type
$\|T\|_n:\Type_n$ generated by
\begin{itemize}
...
\end{lem}
%
We refer the reader to~\cite[7.3]{hottbook} for more details on propositional truncation.
...
% Colimits behave well with truncations, in the following sense:

% \begin{lem}
%   Let $D$ be a diagram, $m$ a truncation index, and
%   $P:\Type_{m}$. Let's call $P$ a $m$-colimit of $D$ if
%   for any other $X:\Type_m$, the
%   map
%   \begin{mymath}\lambda f:P \to X, \left( \lambda i,~f \circ q_i\, ;\, \lambda i\, j\,
%   \phi,\ \precompose_f (p_{i, j}^\phi)) \right)\end{mymath}
%   is an equivalence.
...
%   is $m$-truncated, $P$ is a $m$-colimit of $\|D\|_m$.
% \end{lem}
...
\begin{mymath}\modal \left(\sum_{x:X}  (f x = y)\right) = \sum_{x:\modal X}
...
\begin{defi}
  ...
  %
  \begin{mymath}\prod_{e:E}~ \left(\sum_{e':E} e=e'\right) \simeq \modal_n
  (\chi~e).\end{mymath}
  % 
...
\begin{defi}
  For any type $A$ and $E$, $\iota : A \to E$ and $F:\Type_{n+1}$, we define
    ...
    arrow $f:E\to F$ to its restriction $f \circ \iota$.
...
With topos theory point of view, it means that given a map $A\to F$,
...
\begin{prop}
  ...
  \begin{mymath}\mathrm{separated}(F) = \prod_{x,y:F}~\modal_n(x=y) = (x=y).\end{mymath}
\end{prop}
...
In topos-theoretic words, it means that given a map $A\to F$, one can
...
\begin{proof}
  ...
  We define $g$ as 
  \begin{mymath}g(e) = \modal_n \left( \fib \phi e \right), \end{mymath}
  where
  \begin{mymath}\phi : \left(\sum_{b:\left(\sum_{e:E} (\chi\, e)\right)} (f\,
    b)\right) \to E\end{mymath}
...
\begin{proof}
  ...
  \item For $n=-1$: Let $T:\HProp$. Then
    ...
    \begin{mymath}\psi (u;x) = \eta_T a.\end{mymath}
...
\begin{exm}
  ...
  $i=-1$, for any type $X$:
  \begin{align*}
  \modal X &\defeq \lnot\lnot \|X\|  \\
           &= \lnot\lnot X
  \end{align*}
  by univalence for h-propositions.

 Thus, $\modal$ is exactly the double negation modality on $\Type$
...
indeed, for example, the term 
\begin{mymath}\left( \lambda P:\HProp,~\left\{\begin{array}{l} 0 \text{ if $P$ is
                                    false} \\ 1 \text{ if $P$ is
                                    true} \end{array}\right. \right)
                            P~:~\nat\end{mymath}
will not reduce to a numeral if the chosen property $P$ is not
decidable. However, we keep canonicity for ``old'' terms, i.e. the
...
% indexed by $\nat$, for example with the family $C_n \defeq \text{«\,The $n$-th Turing Machine halts\,»}$.
...
Another issue with universe polymorphism lies in the management of
...
\item 
  ...
  one. But some types in homotopy type theorey are not
