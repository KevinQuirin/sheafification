...
$$\lambda f:P \to X, \left( \lambda i,~f \circ q_i\, ;\, \lambda i\, j\,
  \phi,\ \precompose_f (p_{i, j}^\phi)) \right)$$
...
\begin{defi}
  ...
  %
  $$\prod_{e:E}~ \left(\sum_{e':E} e=e'\right) \simeq \modal_n
  (\chi~e).$$
  % 
...
\begin{prop}
  ...
  $$\mathrm{separated}(F) = \prod_{x,y:F}~\modal_n(x=y) = (x=y).$$
\end{prop}
...
\begin{proof}
  ...
  \begin{lem}
    ...
    \item If $g$ is an embedding, then 
      $$\prod_{b:B}\fib f b = \fib {g\circ f} {g\, b}.$$
    \item If all fibers of $f$ and $g$ are $n$-truncated, then
      $$\prod_{c:C}\modal_n(\fib {g \circ f} c) =
      \modal_n \left(  
...
$$\left( \lambda P:\HProp,~\left\{\begin{array}{l} 0 \text{ if $P$ is
                                    false} \\ 1 \text{ if $P$ is
...
% $n:\nat$, the type $$C_n \defeq \sum_{k:\nat}\left( c^k(n) = 1 \land k\text{ is the
%   smallest such $\nat$}\right)$$ is an $\HProp$.
