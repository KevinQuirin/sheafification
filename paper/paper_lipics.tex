...
\begin{mymath}[-0.5em]\IsEquiv\left(\lambda f:P \to X, \left( \lambda i,~f \circ q_i\, ;\, \lambda i\, j\,
...
Now, let $Q$ be any separated $\Type_{n+1}$, and $f:X \to Q$. Then this
 diagram commutes
...
%
We note that for this solution to work, we need to have a slighlty
weaker definition of modalities than the one in~\cite{hottlib}. We
usually ask the reflector $\modal$ to map a universe $\Type^i$ to
itself, while here, we need $\modal T$, when $T:\Type^i$ to live in a
possibly higher universe $\Type^j$. For modalities on whole $\Type$,
we checked that all properties are still satisfied (see
\url{https://github.com/KevinQuirin/HoTT/extended_modalities/} for the
formalization).
