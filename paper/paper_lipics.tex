...
\author[1]{Kevin Quirin}
\author[2]{Nicolas Tabareau}
\affil[1]{École des Mines de Nantes, France\\
  \texttt{kevin.quirin@mines-nantes.fr}}
\affil[2]{Inria, France\\
  \texttt{nicolas.tabareau@inria.fr}}
\authorrunning{K. Quirin and N. Tabareau} %mandatory. First: Use abbreviated first/middle names. Second (only in severe cases): Use first author plus 'et. al.'

\Copyright{Kevin Quirin and Nicolas Tabareau}
%mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/

\subjclass{F.4.1 Mathematical Logic and Formal Languages}% mandatory: Please choose ACM 1998 classifications from http://www.acm.org/about/class/ccs98-html . E.g., cite as "F.1.1 Models of Computation". 
\keywords{sheaf, homotopy type theory, modalities}% mandatory: Please provide 1-5 keywords
% Author macros::end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %
% %
% Section~\ref{ssec:translation} defines the translation of Matin-Löf
...
topos theory and then give an overview of our construction.
...
dense subobject $A$ of $E$, the canonical map $\Hom{\E}(E,F) \rightarrow \Hom{\E}(A,F)$ is an
isomorphism.
%
...
\begin{equation}\label{eq:induction} \begin{array}{l}
...
For instance, in the notion of separated objects defined in section~\ref{sec:sheafification},
%(which is a technical
%notion used in the construction of the sheafification functor), \ie
%objects for which the morphism
%(\ref{equ:sheaf_def}) is just an embedding, 
subobjects are generalized in two
different ways.  
%
The subobjects to be considered are given by $n$-truncated homotopy fibers,
while the morphism $\Hom{\E}(E,F) \rightarrow \Hom{\E}(A,F)$ is still required to be an
embedding.
...
\begin{mymath}[-0em]\IsEquiv\left(\lambda f:P \to X, \left( \lambda i,~f \circ q_i\, ;\, \lambda i\, j\,
  \phi,\, f (p_{i, j}^\phi)) \right)\right).\end{mymath}
\end{defi}
...
\begin{mymath}[-0em]\modal \left(\sum_{x:X}  (f(x) = y)\right) = \sum_{x:\modal X}
...
Another example of a modality for every level $n$ is the
...
For space reason, the main ideas of the proofs are given in the
appendix, and we refer the reader to the formalization for all details.
...
a type seen as a $n$-subobject of its closure is dense.
...
Equivalently, it can be checked that a type is separated if, and only if all its path types
are $n$-modal. 
...
\begin{cond}\label{cond:hprop}
  ...
  $\Type_n$),  $\modal_n \widehat P \simeq \modal_{-1} P$ and the
  following coherence diagram commutes
  \begin{mymath}[-0em]\xymatrix{
...
In the rest of this section, we suppose given a truncation index
...
\separated T \defeq  \im (\lambda~t:T,~\lambda~ t',~ \modal_n (t = t')).
 %          \left( =  \sum\limits_{u:T \to \Type_n^\modal} \left\| \sum_{a:T} 
  %          (\lambda t,~\modal_n (a=t)) = u\right\|\right).
%\end{array}
\end{mymath}%
%
...
We already know that $\separated T$ is the $(n+1)$-colimit of
$\|C(\mu_T)\|_{n+1}$. We can show, with an encode-decode method, that
a embedding-like property is preserved by $\widetilde\cdot$: 
\begin{equation}\label{eq:Omono}
  \mathrm{if}~\forall a,b:A,~f (a) = f (b) \simeq \modal (a=b),
\end{equation} 
\vspace{-2em}
\begin{mymath}
  \mathrm{then}~\forall a,b:\|T_f\|_{n+1},~|\tilde f|_{n+1} (a) = |\tilde
  f|_{n+1} (b) \simeq \modal (a=b)
\end{mymath}
This can be used to show that the two diagrams $\|C(\mu_T)\|_{n+1}$ and
$\mathring T$ are equivalent, and thus
...
% We know that $\separated T$ is the $(n+1)$-colimit of the iterated
% kernel pair of $\mu_T$.
...
Now, let $Q$ be any separated $\Type_{n+1}$, and $f:X \to Q$. 
Then $|f|_{n+1}$ can be extended to build a cocone over $\mathring
T$ into $Q$.

% Then this
%  diagram commutes
% \begin{mymath}[-0.5em]\xymatrix{
% \|X\|_{n+1} \ar[r] \ar[rd]_{|f|_{n+1}} & \|\mathring T_{X}\|_{n+1} \ar[r] \ar[d] & \|\mathring
...
%
This is enough to prove that $(\separated,\mu)$ defines a reflective
subuniverse on $\Type_{n+1}$.
As $(\separated,\mu)$ is a modality and closure is universal, we can state
...
$T\to \Type_n^\modal$, $\modal_{n+1}T$ is a sheaf. We can now state:

\begin{prop}\label{prop:Sn-modality}
...
7.11]{hottbook}, $\modal$ isn't either. But when
restricted to $i$-truncated types, $\modal = \modal_i$, so:
\begin{lem}
...
transformation, as done in the appendix (section~\ref{ssec:translation}).
...
\url{https://github.com/KevinQuirin/sheafification}. This development
contains 7914 lines, with approximatively 20\% lines of
definitions, which seems quite reasonable and encouraging, because it
suggests that homotopy type theory provides a convenient tool to
formalize some part of the theory of higher topoï.
%
...
\paragraph*{On the use of universe polymorphism}
%\subsection{Limitations of the formalization}
%\label{sec:limit-form}

The development uses the new universe polymorphism feature of
Coq~\cite{sozeau2014universe}. However, it still lacks some features, and
in the formalization, we had to use the \texttt{type-in-type} option, to handle
the universe issues we faced. If not entirely satisfactory, this option
is a convenient way to check the whole construction in Coq, but for
universes.
%
The following universe issues have being checked on paper, like in the
Unimath
library.\footnote{\url{https://github.com/UniMath}, {Vladimir Voevodsky, Benedikt Ahrens, Daniel Grayson and others}}
...
recursive definitions. Indeed, the recursive definition of
sheafification~(\ref{eq:induction}) is not allowed. 
...
%
So universe issues in our formalization may be solved by
generalizing the management of universe polymorphism for recursive definition
%
and also by using of general ``resizing axioms'' which are still under
discussion in the community.
%
We note that for this solution to work, we need to have a more general 
definition of modalities than the one in~\cite{hottlib}, which ask the
reflector $\modal$ to map a universe $\Type^i$ to
itself. Here, we relax this constraint by allowing the reflector to
map a universe $\Type^i$ to a
possibly higher universe $\Type^j$. 
We have checked that the library~\cite{hottlib} still works with this
generalized definition after minor modifications (see
\url{https://github.com/KevinQuirin/HoTT/tree/extended_modalities/} for the
formalization).
...
% \end{itemize}
%
(iii) In topos theory, Lawvere-Tierney subsumes Grothen\-dieck~\cite[Section~V.4]{maclanemoerdijk} in the sense that any
...
\appendix
\section{Proofs}
As said in section~\ref{sec:sheafification}, some proofs were ommited
in the paper, due to space reason. The reader can find the key points
here, and see the complete proofs in the Coq formalization, at
\url{https://github.com/KevinQuirin/sheafification}.


\begin{proof}[Proof of Proposition~\ref{prop:sheaf-is-sheaf}]
...
\begin{proof}[Proof of Proposition~\ref{prop:sheaf-forall}]
  \begin{itemize}
...
\begin{proof}[Proof of Proposition~\ref{prop:sep-mod}]
  Only point {\it (v)} of the definition remains to be checked.

  Let $A:\Type_{n+1}$ be a separated type and $B:A \to \Type_{n+1}$ be a
family of separated types. We want to show that $\sum_{x:A} (Bx)$ is separated. Let $E$
...
\begin{mymath}\xymatrix @R=4em @C=4em{
  \sum_{e:E} (\chi\, e) \ar@<-2pt>[r]_{g\circ\pi_1} \ar@<2pt>[r]^{f\circ \pi_1} \ar[d]_{\mathrm{dense}}& \sum_{x :A} (B\,x) \\
  E \ar@<-2pt>[ru]_{g} \ar@<2pt>[ru]^{f}&
}\end{mymath}%
We can restrict the previous diagram to 
\begin{mymath}\xymatrix @R=4em @C=5em{
  \sum_{e:E} (\chi\, e) \ar@<-2pt>[r]_{\pi_1\circ g\circ\pi_1} \ar@<2pt>[r]^{\pi_1\circ f\circ \pi_1} \ar[d]_{\mathrm{dense}}& A \\
...
\begin{proof}[Proof of Proposition~\ref{prop:Sn-modality}]
  ...
  can prove that $\modal_{n+1}$ preserves dependent sums.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:compatibility}]
...
\begin{proof}[Proof of Proposition~\ref{prop:univalence}]
...
In section~\ref{ssec:overview}, we announce that sheafification can be
seen a translation of type theories\footnote{We define the translation on Martin-Löf type theory,
  but it could also be done for CIC as well.}, as it has been done for
forcing~\cite{jaber2012extending}. Actually, this is not a specific
property of sheafification, but something shared by all modalities. We
hence don't explicit it in the paper, but only here in this appendix.
...
\left[ \sum_{x:A} B \right] &\defeq&  \left( \sum_{x:\Lbrack A \Rbrack}
                                ...
                                \right)\\
  \left[  (x;y)\right] &\defeq& ([x];[y]) \\
  \left[  \pi_i t\right] &\defeq& \pi_i [t] \\
\end{array}
\end{mymath}
...
\left[  A=B \right] &\defeq& \left( [A] = [ B] , \pi_=^{[A],[B]}
                             \right)\\
\left[ 1 \right] &\defeq& 1\\
\left[ J \right] &\defeq& J \\
\end{array}
\end{mymath}
\item For sums
\begin{mymath}
...
\left[  A+B \right] &\defeq& \left( \modal(\Lbrack A \Rbrack + \Lbrack B
                        ...
                        \Rbrack)\right)\\
\left[  \mathrm{in}_\ell t \right] &\defeq& \eta (\mathrm{in}_\ell [t]) \\
\left[  \mathrm{in}_r t \right] &\defeq& \eta (\mathrm{in}_r [t]) \\
...
\begin{array}{lcl}
  \left[  \|A\|_i \right] &\defeq& (\modal \| \Lbrack A\Rbrack  \|_i;
                                   ...
                                   \|_i)) \\
  \left[ |t|_i \right] &\defeq& \eta |[t]|_i \\
...
therefore have to use $J$, the eliminator of the
