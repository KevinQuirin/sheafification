...
Another useful properties of modalities is the following~:
\begin{prop}
...
an inhabitant of a type $T$ is actually an inhabitant of $P \to
T$. Thus, being in this subuniverse is just adding the axiom $P$ to
the theory. Sadly, adding an axiom using the modality does not add any
computational content to $P$, thus it is not completely satisfactory.
...
colimit of $C(f)$ is $Y$. Viewing homotopy type theory as a
type-theoristic version of higher topos theory, it seems that this
axiom can be proved~\cite{lurie}, although we admit it in this paper.
