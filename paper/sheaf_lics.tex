...
The sheaf construction, or sheafification~\cite{maclanemoerdijk}, is a
very powerful tool to construct new topoi from already existing ones,
allowing logical principles---that can not be proven to be true or false
in the old topos---to be valided (or invalided) in the new topos.
%
The most common use of sheafification is to construct a boolean
topos, thus validating the classical principle of the law of excluded
middle (EM), from an intuitionistic topos. 
%
An other famous (revisited) application has been developed by Paul
...
in presence of the axiom of choice (AC).

Even if similar questions have been considered around the Curry-Howard
isomorphism, there is no canonical way to extend a programming language
such as type theory while keeping consistency automatically. 
%
For instance, much effort have been done to provide a computational
content to the law of excluded middle in order to define a constructive
version of classical logic. This has lead to various calculi, with
most notably the $\lambda \mu$-calculus of
Parigot~\cite{parigot1993classical}, but this has not allow to define
a new version of type theory with classical principles.
%
...
Nevertheless the axiom of choice Coquand, Herbelin, Krivine.
%
% Some works have tried to provide a realization of the axiom of choice 
