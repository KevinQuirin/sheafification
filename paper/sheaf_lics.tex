...
can be replaced by Giraud's axiom which basically allows to replace 
 kernel pairs by \v{C}ech nerves in the property.
...
and presents a particular case of GIraud's axiom. 
%
...
$$ 
%
A function $f$ is with $n$-truncated homotopy fibers when $\fib{f}{b}$
...
\IsType n~\fib{f}{b}\right) \simeq (B \to \Type_n)$$
%
and a pullback (with $\Type_n^\bullet \defeq \sum_{A:\Type_n} A$ the universe of pointed
$n$-truncated types)~:
$$
\xymatrix{
...
$$
for any $f$ with $n$-truncated homotopy fibers, with 
...
In the definition of (a special case of) Giraud's axiom given in
...
An colimit of a diagram $D$ over a graph $G$ is given by a type $P$
that defines a cone of $D$, plus the universal property that for any
type $X$, the canonical map that transforms a function from $f : P
...
actually a colimit over $D$, we do not detail the proof as the
...
Moreover, left exactness implies in particular homotopy fibers preservations~:
\begin{prop}
