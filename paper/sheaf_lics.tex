...
of $T$.
\begin{defi}
  Let $f$ be a map from $X$ to $Y$. Then $C(f)$
...
Now, we have defined a (countably) infinite family of modalities
$\Type_i \to \Type_i$. Now:

\begin{lem}
  Let $\modal_i:\Type_i \to \Type_i$ be a modality. Then $\modal_i
...
universe where the forced principle ({\em e.g.} mere excluded middle) is
...
%
\item Colimits and \v{C}ech nerves are formalized in
\texttt{Limit, T.v, OT.v OT\_Tf.v, T\_telescope.v, Tf\_Omono\_sep.v}.% (552 lines).
\item
...
In the formalization, we had to use the type-in-type option, to handle
