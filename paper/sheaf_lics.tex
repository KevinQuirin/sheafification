...
minor changes), but they can be found in our Coq form 
...
\item 
  Let $A:\Type$ and $B:A \to \Type_n^\modal$. Then $P\left(\prod_{a:A} (B\,
  a)\right)$ (from \cite[Lemma 7.7.2]{hottbook}). 
\item For any $A,B:\Type_p$,
  $\modal (A\times B) = (\modal A) \times (\modal B)$ \\
(from \cite[Corollary 7.7.2]{hottbook}).
\item Let $A:\Type_n \modal$. Then for any $x,y:X$, $P(x=y)$.
\item 
  {\it (iv)} can be extended to dependent product, that is for any $A:\Type_p$, $B:\modal A \to
  \Type_p^\modal$ and
