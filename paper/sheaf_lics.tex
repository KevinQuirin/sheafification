...
\label{sec:homotopy-type-theoryhott}
...
\begin{defi}
  ...
  \item For all $A~:~\Type_p$ and $B~:~A \to \Type_p$ such that $P(A)$
    and $\Pi_{x:A} P(B x)$, then $P\left( \Sigma_{x:A} B(x)\right)$
  \item For all $A~:~\Type_p$ and $x,y~:~A$, if $\modal A$ is
    contractible, then $\modal (x=y)$ is contractible.
  \end{itemize}
\end{defi}

All theorem in~\cite{hottbook}, chapter 7.7 remains true + fibers
preservation :

for all $n$-truncated types,
  and all map $f~:~X \to Y$, the modalisation of fiber of $f$ above any element $y:Y$
  is the fiber of $\modal f$ above $\eta y$~:
  $$\modal \{x:X ~\&~ f~ x = y\} = \{x:\modal X ~\&~ \modal f ~x = \eta_Y
  y\}.$$

  We want moreover that the following diagram commute
  $$\xymatrix{
    \{x:X ~\&~ f~ x = y\} \ar[r]^\eta \ar[d]_\gamma & \modal \{x:X ~\&~ f~ x = y\} \ar@{=}[dl]\\
    \{x:\modal X ~\&~ \modal f ~x = \eta_Y y\} &
  }$$
...
We try to define sheaves inductively~: we define sheaves in $\HProp$
as the classical types ({\em ie} implied by their double negation), and
we define sheaves on $\Type_{n+1}$ {\em via} usual definition of sheaves,
as seen in~maclanemoerdijk and sheaves on $\Type_n$. Of course,
the definition must be cumulative.

We will call $\mathcal S_i$ the universe of sheaves on $\Type_i$.

\subsection{Definitions and forst properties}
\label{sec:def}

\begin{itemize}
\item Density
\item Separated Type / Sheaf / closure : a subobject is always dense
  into its closure.
\item $\Omega_n^\modal$ is a sheaf.
\item Sheaves are stable by dependent products.
\end{itemize}

\subsection{Construction of sheafification}
\label{sec:sheafification}

We mimic the construction in~\cite{maclanemoerdijk}
Two steps~: separation, sheafification.

\subsubsection{For h-propositions}
\label{sec:h-propositions}

\begin{itemize}
\item Any left exact modality on $\HProp$.
\item We use $\lnot\lnot$
\end{itemize}
\subsubsection{From Type to Separated Type}
\label{sec:from-type-separated}

\begin{itemize}
\item Definition of the separation reflective subuniverse
  $(\square,~\mu)$.

  $\square$ + $\square T$ is separated for all $(n+1)$-truncated
    type $T$ 
    \begin{thm}
...
\item If $a,b~:~T~: \Type_{n+1}$ such that $\mu_T a = \mu_T b$, then
  $\modal (a=b)$.

\item Cech nerve of $\mu$ is the diagram of closure of diagonals,
  hence their colimits are equal. Since colimits are universal, so is $\square$.

\item $(\square,~\mu)$ is a modality.

\end{itemize}


\subsubsection{From Separated Type to Sheaf}
\label{sec:separated-to-sheaf}

\begin{itemize}
\item 
\begin{thm}
  Any type $A~:~\Type_{n+1}$ closed in a sheaf is itself a sheaf.
\end{thm}

\item Definition of sheafification reflextive subuniverse $(\star,~\nu)$
\end{itemize}
