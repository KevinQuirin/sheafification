...
with propositional classical logic. 
...
\item Suppose $p : (\mu_T a = \mu_T b)$. Then projecting $p$ along
  ...
  $.
  Taking for example $t=b$, we deduce $\modal_n (a=b) = \modal_n(a=a)$,
...
$$\modal_{n+1} T = \sum_{u:T \to \Type_n^\modal} \modal_{-1}\left\| \sum_{a:T} 
            (\lambda t,~\modal_n (a=t)) = u\right\|$$
...
\begin{proof}
  ...
  \item For $n=-1$: Let $T:\HProp$. Then
    ...
    witness $a:T$, and letting
    $$\psi (u;x) = \eta_T a.$$
    These two maps form an equivalence.
  \item Suppose now that $\modal_{n+1}$ is compatible with all $\modal_k$ on
    ...
    $T:\Type_{n+1}$. Then
    \begin{align*}
      \modal_{n+2} T &= \sum_{u:T \to \Type_{n+1}^\modal} \modal_{-1}\left\| \sum_{a:T} 
            (\lambda t,~\modal_{n+1} (a=t)) = u\right\| \\
                     &= \sum_{u:T \to \Type_{n+1}^\modal} \modal_{-1}\left\| \sum_{a:T} 
            (\lambda t,~\modal_{n} (a=t)) = u\right\|
    \end{align*}
    because $\modal_{n+1}$ is compatible with $\modal_{n}$, and
    $(a=t):\Type_n$.
    
    It lefts to prove that $u:T\to\Type_n^\modal$, \ie that for all
    $t:T$, $\IsType n (u\, t)$.
    But for any truncation index $p$, the predicate $\IsType p:\HProp$
    is a a sheaf, so we can get rid of $\modal_{-1}$, and pick up what
    is inside the truncation. Thus, we know that for all $t:T$, $u\, t
    = \modal_n(a=t) : \Type_n$.
  \end{itemize}
\end{proof}
...
$n-\Type$, it suffices to show that $\modal_n(x=y)$ is contractible.
...
\texttt{sheaf\_induction.v}), three very technical facts required to
show that $\square$ and $\modal_{n+1}$ are modalities have been
admitted. Nevertheless, we are confident that they can be proven and
...
%
When the absence of cumulativity rises an universe issue for a lemma,
