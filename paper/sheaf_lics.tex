...
\end{lem}
%
As $\sum_{x:\modal X} (\modal f x = \eta_Y y)$ is modal, we
...
particular, this property uses in particular the specific definition
...
$$
\begin{array}{rcl}
\separated T &\defeq & \im (\lambda~t:T,~\lambda~ t',~ \modal (t = t')) \\
          & \defeq & \sum_{u:T \to \Type_n^\modal} \left\| \sum_{a:X} 
...
To show that $\separated$ is a modality, we need t show that $\separated T$
...
\begin{proof}
  ...
  But as $\star T$ is the closure of $\separated T$, $\separated T$ is dense
  into $\star T$, so the sheaf property of $Q$ allows to extend
  $\separated f$ to $\star f:\star T \to Q$.
