...
\subsection{The epi-mono Factorization System}
\label{sec:epi-mono-fact}

Now, let a map $f:A\to B$ be called {\em (-1)-truncated} if
every fibers of $f$ is an $\HProp$, and {\em
  (-1)-connected} if every fibers $f$ is merely
inhabited, \ie such that $\|\fib f y\|$ for all $y$.

\begin{defi}
  Let $f:A\to B$ be a function. The {\em image} of $f$ is
  $$\im(f) = \sum_{y:B} \|\fib f y\|.$$
\end{defi}

It is then fairly obvious that the canonical function $\hat f : A \to
\im(f)$ is (-1)-connected, thus the following proposition arise
(\cite{hottbook}, Lemma 7.6.4)~:
\begin{prop}
  An map $f:A\to B$ factors as a (-1)-connected function followed by a
  (-1)-truncated function.
\end{prop}

\nt{define the image here, with (-1)-truncated and (-1)-connected
...
\item $\one$ seen as a $\Type_p$ is modal, ie $\modal \one =\one$.
\item (\cite{hottbook}, Lemma 7.7.2)
  Let $A:\Type$ and $B:A \to \Type_p^\modal$. Then $P\left(\prod_{a:A} (B\,
  a)\right)$. 
\item (\cite{hottbook}, Corollary 7.7.2) For any $A,B:\Type_p$,
  $\modal (A\times B) = (\modal A) \times (\modal B)$.
\item Let $A:\Type_p\modal$. Then for any $x,y:X$, $P(x=y)$.
\item (\cite{hottbook}, Theorem 7.7.4) 
  {\it (iv)} can be extended to dependent product. Precisely~:

  Let $A:\Type_p$ and $B:\modal A \to \Type_p^\modal$. For any map
...
The central result used here is the following
\begin{lem}
  Let $X:\Type_p$, $Y:\Type_p\modal$ and $f:X\to Y$. If for all $y:Y$,
  $\modal (\fib f y)$ is contractible, then $\modal X = Y$.
\end{lem}

Here, $\sum_{x:\modal X} (\modal f x = \eta_Y y)$ is modal, thus we
can define $$\phi:\modal \left(\sum_{x:X}  (f x = y)\right) \to
\sum_{x:\modal X} (\modal f x = \eta_Y y).$$
We just need to check that the image of its fibers by $\modal$ are
contractible.
...
\end{lem}
We can rewrite $\fib \phi {x;p}$ in terms of the fiber of a fonction
$\fib {\eta_X} a \to \fib {\eta_Y} b$.
...
Finally, $\modal(\fib \phi {x;p})$ is contractible, and the result is proved.
\kq{Finish that}
\end{proof}
...
\nt{Il faut mieux introduire ces propriétés, $\Type_n^\modal$ is a sheaf
...
\end{prop}
\begin{proof}
  Firstly, we prove that $\Type_n^\modal$ is separated. Let $E:\Type$
  and $\chi:E \to \Type_n$, dense in $E$. Let $\phi_1,\phi_2:E \to
  \Type_n^\modal$, such that $\phi_1 \circ \pi_1 = \phi_2 \circ
  \pi_1$. Finally, let $x:E$. We show $\phi_1 x = \phi_2 x$ using
  univalence.

  As $\chi$ is dense, we have a term $m_x : \modal(\chi\, x)$.
  But as $\phi_2 x$ is modal, thus we can obtain a term $h_x : \chi\,
  x$. 
...
induction the truncation index $n$, a
...
Note that once the first modality is defined, the extension to all
$\Type_n$ is automatic. This means that the new logical principles
that can be added by the sheaf construction must be added in the
first modality already. The sheafification is just a way to propagate
those new principles to other types than just mere proposition. 
...
Let $T : \Type_{n+1}$. We define $\square T$ as the image (see
...
(t':T),~t=t'$.
\nt{parler du système de factorization}
...
\begin{proof}
  As $\mu_T$ is an surjection, using the particular case of the last
  Giraud-Rezk-Lurie axiom introduced in Section~\ref{sec:giraud-ax},
  we know that $\square T$ is the colimit
...
Some other lemmas are proved, then admitted. We do it so that Coq
doesn't constrain too much universes, without having to explicitly
annotate universes everywhere in the formalization.

The main issue in this formalization is the gestion of universes in
Coq~\cite{sozeau2014universe}. If Coq can handle cumulativity on all
$\Type$, it is not the case for the sigma type $\Type_n$. This issue
can be solved by adding an axiom of cumulativity for $\Type_n$, but as
it would not have any computational content, it would really
complicates the proofs.

Moreover, even the definition of successive sheafification types is
not allowed~:
...
\[ \left\{
    ...
    \end{array}
  \right.,\]
because Coq asks for every $\modal_n$ to have the same universes levels.

Talk about issues in universe levels \cite{sozeau2014universe}. Lake of cumulativity for
...
Prove that the Giraud-Rezk-Lurie axiom holds in homotopy type theory,
or at least the particular case we need.
