...
\usepackage{wasysym}
\usepackage[all]{xy}
\def\dar[#1]{\ar@<2pt>[#1]\ar@<-2pt>[#1]} %double arrows in xy
\def\tar[#1]{\ar@<2pt>[#1]\ar@<0pt>[#1]\ar@<-2pt>[#1]} %triple arrows in xy
...
\newcommand{\IsType}[1]{\mathrm{Is-}#1\mathrm{-type}}
...
$$\xymatrix{ 
    \cdots \overline{\Delta_3} \tar[r] \ar[rd]_{f\circ \pi_1} & \overline{\Delta_2}
    \dar[r] \ar[d]^{f\circ \pi_1} &
...
$$\xymatrix{
  \sum_{e:E} (\chi\, e) \dar[r]^{f\circ\pi_1}_{g\circ \pi_1} \ar[d]_{\mathrm{dense}}& \sum_{x :A} (Bx) \\
  E \dar[ru]^f_g&
}$$
We can restrict the previous diagram to 
$$\xymatrix{
  \sum_{e:E} (\chi\, e) \dar[rr]^{\pi_1\circ f\circ\pi_1}_{\pi\circ g\circ \pi_1} \ar[d]_{\mathrm{dense}}&& A \\
  E \dar[rru]^{\pi_1\circ f}_{\pi_1\circ g}&
},$$ 
...
$$\xymatrix{
  \sum_{y:\sum_{e:E} (\chi\,
  e)} x=y_1 \dar[rrr]^{\pi_2\circ f\circ\pi_1\circ \pi_1}_{\pi_2\circ g\circ \pi_1\circ \pi_1} \ar[d]_{\mathrm{dense}}&&& B\,x \\
  \sum_{y:E} x = y \dar[rrru]^{\pi_2\circ f\circ \pi_1}_{\pi_2\circ g\circ \pi_1}&
}$$
