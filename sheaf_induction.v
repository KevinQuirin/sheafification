...
(*   apply univalence_axiom. *)
(*   exists (λ (χ:E->J), existT (λ φ, forall e:E, {e':E & e=e'} = (Oj (φ e)).1.1) (pr1 ° χ) (J_OK χ)). *)
(*   apply isequiv_adjointify with (g := λ (χ:EJ E) (e:E), existT (λ P:HProp, (j P).1) (χ.1 e) (EJ_ok χ e)). *)
(*   - intro x. unfold composition; simpl. destruct x as [χ dense]. simpl. *)
...
(*     admit. *)
(*   - intro x. apply path_forall; intro u. unfold composition; simpl. *)
(*     rewrite <- eta_sigma. *)
(*     apply eq_dep_sumT with (prf := idpath). *)
...
Section Sheafification.
  ...
  Definition type_j_f E (χ: E -> J) :
    (E -> subuniverse_Type nj) -> pr1 (nchar_to_sub (pr1  ° χ))
    -> subuniverse_Type nj := λ α e, α (pr1 e).

  Definition type_j_inv E (χ: E -> J) : (pr1 (nchar_to_sub (pr1  ° χ)) -> subuniverse_Type nj) -> E -> subuniverse_Type nj :=
  λ α e, let f := (pr2 (nchar_to_sub (pr1  ° α))) in
         let m := (pr2 (nchar_to_sub (pr1  ° χ))) in
         nj.(O) (nsub_to_char n ({b : _ &  pr1 (pr1 (α b))}; ((pr1 m) ° (pr1 f); function_trunc_compo n (pr1 f) (pr1 m) (pr2 f) (fun e => IsHProp_IsTrunc (pr2 m e) n0))) e).
  ...
  Proof.
    ...
    assert (O_rec (χ x) (φ2 x)
     ...
     apply ap. apply functional_extensionality_dep. intro v.
    ...
    apply (transport (λ u, u=y) (id_sym XX)). clear XX; clear X.
  ...
  Lemma nTjTiseparated_eq : separated (subuniverse_Type nj ; @subuniverse_Type_is_TruncSn _ nj).
    ...
    - intro x.
      ...
      (* unfold composition, equiv_inv in foo. *)
  ...
  : (λ (f : E -> ∀ a : A, ((B a) .1) .1) (t : {b : E | ((χ b)) .1}),
    ...
    exact (apD10 (equiv_inv (functional_extensionality_dep _ _ (λ t, apD10 ((apD10 H) t) a))) u).
  Defined.
  ...
  Definition cloture E (χ : E -> Trunc n) : E -> Trunc n := pr1 ° nj.(O) ° χ.
...
(* := λ x:((pr1 T)**(pr1 T)), (fst x = snd x ; istrunc_paths with (H:=T.2)) *)
  ...
  Definition E_to_χ_map_ap (T U:Trunc (trunc_S n)) E (χ : EnJ E) (f : E -> (pr1 T))
    ...
    apf g (ap (E_to_χ_map T χ) e) = ap (E_to_χ_map U χ) (apf g e).
  ...
  Instance separated_mono_is_separated_ (T U:Trunc (trunc_S n)) E χ g h (f: pr1 T -> pr1 U)
        (H:IsEquiv (ap (E_to_χ_map U (E:=E) χ) (x:=f ° g) (y:=f ° h))) (fMono : IsMonof f) : 
 ...
 :=
    ...
    intro X. destruct X as [ab p]. destruct ab as [a b]. simpl in p.
      ...
      assert (inj (separated_unit T a) = inj (separated_unit T b)).
        ...
        - intro x.
          ...
          pose (foo := function_lift_compose). unfold function_lift, composition in foo.
  ...
  Definition E_to_Y'A
             ...
             (E : Type)
             (χ : E -> J)
             (X : {b : E & pr1 ((pr1 (P:=λ b0 : HProp, ~ ~ pr1 b0) ° χ) b)} -> pr1 A)
             (X0 : E)
             (inv_B : (pr1
                         (nchar_to_sub
                            (pr1
                               (P:=λ b : HProp,
                                         pr1 ((pr1 (P:=λ P : HProp, pr1 (is_classical P)) ° Oj) b))
                               ° χ)) -> pr1 (pr1 B)) -> E -> pr1 (pr1 B))
             (retr_B : Sect inv_B (E_to_χmono_map (pr1 B) (χ:=χ)))
             (Y := inv_B (m ° X) : E -> pr1 (pr1 B))
    := (λ b, (pr1 b ; (X b ; (inverse (equal_f _ (retr_B (m°X)) b)))))  : {b : E & pr1 (pr1 (χ b))} -> {b : E & hfiber m (Y b)}.
...
:
  ...
  : ((nchar_to_sub (pr1 ° χ)) .1 -> A .1) -> (E -> A .1).
    ...
    destruct (closed (inv_B ((pr1 m)°X) X0)) as [inv_closed retr_closed sect_closed adj_closed].
    
    exact ((pr1 (P:=_) ° inv_closed) (pr2 (pr1 (pr2 (closure_naturality_fun _ _ (@clE_to_clY'A A B (pr1 m) (pr2 m) closed E χ X X0 inv_B retr_B (X0;X2))))))).
  Defined.
  ...
  : Sect (@closed_to_sheaf_inv A B m closed E χ) (E_to_χmono_map A (χ:=χ)).
    ...
    unfold closed_to_sheaf_inv, E_to_χmono_map, nsub_to_char, composition, hfiber, O_rec, Equivalences.internal_paths_rew in *; simpl in *.
  ...
  : Sect (E_to_χmono_map A (χ:=χ)) (@closed_to_sheaf_inv A B m closed E χ).
    ...
    apply functional_extensionality_dep; intro b.
    unfold E_to_χmono_map, nsub_to_char, composition, hfiber, O_rec in *; simpl in *.
        destruct (@snd (separated
            (@proj1_sig (Trunc (trunc_S n))
               (fun T : Trunc (trunc_S n) => Snsheaf_struct T) B)) (forall (E0 : Type) (χ0 : forall _ : E0, J),
          @IsEquiv
    ...
    set (Y := inv_B (m ° (X ° pr1) ) : E -> pr1 (pr1 B)).
    ...
    assert ((∀ (e : E) (X0 : ((χ e) .1) .1),
      ...
      apply ap.
      clear eq. specialize (adj_B (m°X)). unfold composition in adj_B.
